...
bool TextureLoader::Initialize(DirectDevice*device)
{
	ID3DBlob* vsBytecode;
	ID3DBlob* psBytecode;
	
	if (!device->CompileD3DShader(L"./Shading/TextureCombine.hlsl", "VS_MAIN", "vs_5_0", &vsBytecode))
	{
	...
	if (!device->CompileD3DShader(L"./Shading/TextureCombine.hlsl", "PS_AMB", "ps_5_0", &psBytecode))
	{
	...
	psBytecode->Release();
	if (!device->CompileD3DShader(L"./Shading/TextureCombine.hlsl", "PS_DIFF", "ps_5_0", &psBytecode))
	{
		MessageBox(NULL, "./Shading/Failed to compile texture combining diff pixel shader", "Failed to compile shader code", NULL);
		return false;
	}
	if (FAILED(device->d3dDevice_->CreatePixelShader(psBytecode->GetBufferPointer(), psBytecode->GetBufferSize(), NULL, &diffPixelShader)))
	{
		MessageBox(NULL, "./Shading/Failed to create game material diff pixel shader", "Failed to create shader", NULL);
		psBytecode->Release();
		return false;
	}
	psBytecode->Release();
	if (!device->CompileD3DShader(L"./Shading/TextureCombine.hlsl", "PS_SPEC", "ps_5_0", &psBytecode))
	{
...
bool TextureLoader::CombineTexture(DirectDevice* device, TEXTURE_MAP_TYPE mapType, UINT width, UINT height)
{
...
TextureLoader::FILE_EXTENSION TextureLoader::getExtension(std::wstring texFile)
{
	int i = 1;
	while (i < 256)
	{
...
bool TextureLoader::LoadTextureFromFileAndCombine(DirectDevice* device, std::string textureFile, TEXTURE_MAP_TYPE mapType)
{
...
void TextureLoader::ReleaseAfterload()
{
